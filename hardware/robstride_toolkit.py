...
from robot.root_constant import CAN_CMD_CLEAR_FAULT
...
def parse_data(data: bytes) -> tuple[int, float, float, float, int]:
    ...
    # Convert to degrees
    position_degrees = math.degrees(position_rad)
    velocity_deg_per_sec = math.degrees(velocity_rad_per_sec)
...
def mit_control(
    ...
    velocity_rad_per_sec = math.radians(velocity_deg_per_sec)
...
def _require_can() -> None:
    if can is None:
        raise RuntimeError(
            "python-can is not installed. Install it in your env with: pip install python-can"
        )
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Robstride CAN toolkit")
    ...
    parser.add_argument("--motor-id", type=int, required=True, help="Motor CAN ID")

    sub = parser.add_subparsers(dest="cmd", required=True)
    sub.add_parser("ping_mit")
    sub.add_parser("ping_private")
    sub.add_parser("ping_canopen")
    sub.add_parser("enable")
    sub.add_parser("disable")
    sub.add_parser("clear_fault")
    sub.add_parser("switch_private_to_mit")
    sub.add_parser("switch_mit_to_private")

    p_read = sub.add_parser("read_param")
    p_read.add_argument("--index", type=lambda x: int(x, 0), required=True, help="Parameter index (e.g. 0x7028)")

    p_write = sub.add_parser("write_param")
    p_write.add_argument("--index", type=lambda x: int(x, 0), required=True, help="Parameter index (e.g. 0x7028)")
    p_write.add_argument("--value", type=lambda x: int(x, 0), required=True, help="Integer parameter value")
    p_write.add_argument("--size", type=int, choices=[1, 2, 4], default=4)

    p_ctrl = sub.add_parser("mit_control")
    p_ctrl.add_argument("--kp", type=float, required=True)
    p_ctrl.add_argument("--kd", type=float, required=True)
    p_ctrl.add_argument("--pos", type=float, required=True, help="Position in degrees")
    p_ctrl.add_argument("--vel", type=float, default=0.0, help="Velocity in deg/s")
    p_ctrl.add_argument("--tau", type=float, default=0.0, help="Torque in N.m")
    ...
    ok = False
    try:
        if args.cmd == "ping_mit":
            ok = ping_mit(bus, args.motor_id)
        elif args.cmd == "ping_private":
            ok = ping_private(bus, args.motor_id)
        elif args.cmd == "ping_canopen":
            ok = ping_canopen(bus, args.motor_id)
        elif args.cmd == "enable":
            ok = enable(bus, args.motor_id)
        elif args.cmd == "disable":
            ok = disable(bus, args.motor_id)
        elif args.cmd == "clear_fault":
            data = [0xFF] * 7 + [CAN_CMD_CLEAR_FAULT]
            msg = can.Message(arbitration_id=args.motor_id, data=data, is_extended_id=False)
            bus.send(msg)
            ok = bool(bus.recv(0.1))
        elif args.cmd == "switch_private_to_mit":
            ok = switch_private_to_mit(bus, args.motor_id)
        elif args.cmd == "switch_mit_to_private":
            ok = switch_mit_to_private(bus, args.motor_id)
        elif args.cmd == "read_param":
            out = single_parameter_read(bus, args.motor_id, args.index)
            ok = out is not None
        elif args.cmd == "write_param":
            ok = single_parameter_write(bus, args.motor_id, args.index, args.value, args.size)
        elif args.cmd == "mit_control":
            ok = bool(
                mit_control(
                    bus,
                    args.motor_id,
